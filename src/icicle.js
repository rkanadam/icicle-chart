...
export default Kapsule({
    props: {
        width: {
            default: window.innerWidth, onChange(_, state) {
                state.needsReparse = true
            }
        },
        height: {
            default: window.innerHeight, onChange(_, state) {
                state.needsReparse = true
            }
        },
        orientation: {
        ...
        children: {
            default: 'children', onChange(_, state) {
                state.needsReparse = true
            }
        },
        sort: {
        ...
        excludeRoot: {
            default: false, onChange(_, state) {
                state.needsReparse = true
            }
        },
    ...
    update: function (state) {
        ...
        newCell.append('rect')
            ...
            .on('mousedown', d => {
                state.onMouseDown && state.onMouseDown(d.data);
            })
            .on('mouseup', d => {
                state.onMouseUp && state.onMouseUp(d.data);
            })
            .on('mouseover', d => {
                //d3Event.stopPropagation();
